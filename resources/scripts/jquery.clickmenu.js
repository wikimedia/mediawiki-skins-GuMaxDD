	...
	$.fn.clickMenu = function ( options ) {
		...
		var hideDIV = function ( div, delay ) {
			...
			if ( div.isVisible ) {
				div.timer = setTimeout( function () {
					// remove events
					$( getAllChilds( getOneChild( div, 'UL' ), 'LI' ) )
		...
		var findParentWithClass = function ( elem, searchTag, searchClass ) {
			...
			for ( ; pNode; pNode = pNode.parentNode ) {
				if (
					pNode.nodeType == 1 &&
					pNode.nodeName.toUpperCase() == searchTag &&
					pattern.test( pNode.className )
				) {
					return pNode;
				}
			}
	...
	$.fn.shadowBox = function () {
		return this.each( function () {
			...
			if ( $( this ).css( 'position' ) == 'absolute' ) {
				// if the child(this) is positioned abolute, we have to use
				// relative positioning and shrink the outerbox accordingly to
				// the innerbox
				$( outer ).css( {
					position: 'relative',
